...
/*   Updated: 2025/06/26 13:21:19 by fdurban-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		...
		while (i < PHILO_MAX && philosophers[i].id != 0)
		{
			...
			if (get_time_stamp() - philosophers[i].time_of_last_meal > philosophers[i].time_to_die)
			{
				...
				pthread_mutex_lock(philosophers[i].check_dead);
				if (!*philosophers[i].someone_died)
				{
					*philosophers[i].someone_died = 1;
					printf("%ld %d died\n", get_time_stamp(), philosophers[i].id);
				}
				pthread_mutex_unlock(philosophers[i].check_dead);
				return NULL;
			}
			pthread_mutex_unlock(philosophers[i].meal_mutex);
			i++;
